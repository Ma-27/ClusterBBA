...
- save_csv(dist_df, out_path=None, default_name='Example_3_3.csv') -> None
- plot_heatmap(dist_df, out_path=None, default_name='Example_3_3.csv', title) -> None

脚本用法：
```bash
$ python bjs_distance.py [Example_3_3.csv]
```
若文件不存在会给出提示。

示例：
```python
from bjs_distance import load_bbas, distance_matrix, save_csv, plot_heatmap
import pandas as pd

df = pd.read_csv('data/examples/Example_3_3.csv')
bbas, _ = load_bbas(df)
df_dist = distance_matrix(bbas)
save_csv(df_dist)                        # 默认使用 Example_3_3.csv 构建路径
...
def distance_matrix(
    ...
    for i in range(size):
        for j in range(i + 1, size):
            mat[i][j] = mat[j][i] = bjs_divergence(bbas[i][1], bbas[j][1])
...
def save_csv(
    ...
    if out_path is None:
        ...
        fname = f"bjs_{os.path.splitext(default_name)[0]}.csv"
...
def plot_heatmap(
    ...
    if out_path is None:
        ...
        res = os.path.join(base, '..', 'experiments_result')
        os.makedirs(res, exist_ok=True)
        fname = f"bjs_{os.path.splitext(default_name)[0]}.png"
        out_path = os.path.join(res, fname)
    fig, ax = plt.subplots()
