...
def train_ova_svm(X_tr_cal: np.ndarray, y_tr_cal: np.ndarray, X_val: np.ndarray, y_val: np.ndarray,
    ...
    # 随机打乱索引并按 1/2 划分为 SVM 训练集与校准集
    rng = np.random.default_rng(split_seed)
    idx = np.arange(X_tr_cal.shape[0])
    rng.shuffle(idx)
    half = X_tr_cal.shape[0] // 2
    idx_svm, idx_cal = idx[:half], idx[half:]
    X_svm, y_svm = X_tr_cal[idx_svm], y_tr_cal[idx_svm]
    X_cal, y_cal = X_tr_cal[idx_cal], y_tr_cal[idx_cal]
...
def evaluate_logistic_calibration(S_val: np.ndarray, calibrators: List[BinaryEvidentialLogisticCalibration],
                                  y_true: np.ndarray, ) -> Tuple[float, float, np.ndarray]:
    """使用 Logistic 标定法评估验证集。返回 ``(accuracy, macro-F1, y_pred)``。"""
...
def evaluate_multinomial_calibration(S_cal: np.ndarray, y_cal: np.ndarray, S_val: np.ndarray, y_true: np.ndarray,
                                     mc_M: int, n_theta_grid: int, ) -> Tuple[float, float, np.ndarray]:
    """使用多项式校准法评估验证集。返回 ``(accuracy, macro-F1, y_pred)``。"""
    K = S_val.shape[1]
...
def cross_validate(X: np.ndarray, y: np.ndarray, n_splits: int, random_state: int, method: str, mc_M: int,
                   n_theta_grid: int, ) -> Tuple[float, float, float, float, np.ndarray, np.ndarray]:
    """执行交叉验证并返回性能统计以及预测结果。"""
    ...
    for fold, (idx_tr, idx_te) in enumerate(
        ...
        if method.lower() == "logistic":
            acc, f1, y_pred = evaluate_logistic_calibration(S_val, calibrators, y_te)
        else:
            acc, f1, y_pred = evaluate_multinomial_calibration(
                S_cal, y_cal, S_val, y_te, mc_M, n_theta_grid
            )
        # 收集每折的分数与预测
    ...
    return (acc_mean, acc_std, f1_mean, f1_std, np.array(y_true_all), np.array(y_pred_all),)
