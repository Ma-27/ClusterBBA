...
使用 :func:`utility.io_application.load_application_dataset` 读取``kfold_xu_bba_seeds.csv``，对每个样本的多条 BBA 按指定融合规则组合，再经 Pignistic 转换得到预测类别，最终计算准确率和 F1 分数。
...
def evaluate_accuracy(*, samples: List[tuple[int, List[BBA], str]] | None = None, debug: bool = False,
    ...
    """计算在 Seeds 数据集上的分类准确率."""

    if samples is None and csv_path is None:
        csv_path = (
                Path(__file__).resolve().parents[1] / "data" / "kfold_xu_bba_seeds.csv"
        )
...
if __name__ == "__main__":
    ...
    if args.method == "Proposed" and args.kfold:
        ...
        params_path = (
            ...
            # / "best_params_kfold_seeds.csv"   # 修改这里以启用旧版本的最优化超参数
        )
