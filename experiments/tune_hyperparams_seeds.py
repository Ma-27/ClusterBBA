# -*- coding: utf-8 -*-
"""在 Seeds 数据集上搜索 ``lambda`` 与 ``mu`` 超参
============================================

遍历候选 ``lambda`` 和 ``mu`` 组合, 调用 :func:`experiments.application_seeds.evaluate_accuracy` 评估分类准确率, 输出前 ``len(candidates)`` 组最优参数.  提供 ``--debug`` 选项可仅尝试前两组参数以快速验证流程.
"""
...
from experiments.application_seeds import evaluate_accuracy
