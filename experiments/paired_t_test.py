...
本脚本使用 5 折交叉验证得到的准确率，对任意两种证据融合方法进行配对样本`t` 检验，输出 t 统计量和对应的 p 值。数据集、待比较的方法以及是否启用按折超参数均可通过命令行参数指定。
"""
...
def _evaluate_method(samples: List[Tuple[int, List, str, int]], method_name: str, *, label_map: Dict[str, str],
                     methods: Dict[str, object], use_kfold_params: bool, params_file: Path, lambda_val: float | None,
                     mu_val: float | None, alpha_val: float | None) -> List[float]:
    """计算给定方法在各折上的准确率列表."""
...
def paired_t_test(dataset: str = "iris", method1: str = "Proposed", method2: str = "Proposed", *,
    ...
    """执行配对 t 检验并返回 ``(t_statistic, p_value)``。默认在单侧检验下判断 ``method2`` 是否显著优于 ``method1``。"""
    ...
    # 使用 scipy 进行配对样本 t 检验
    # 这里将 accs2 置于 accs1 之前, 以便在默认单侧检验时，统计量为正表示 ``method2`` 的表现优于 ``method1``
    alt = "two-sided" if two_sided else "greater"
    t_stat, p_val = ttest_rel(accs2, accs1, alternative=alt)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="配对 t 检验")
    # --dataset：指定需要进行显著性检验的数据集名称
    ...
    # 执行配对 t 检验
    t_stat, p_val = paired_t_test(dataset=args.dataset, method1=args.method1, method2=args.method2,
    ...
    print(f"Paired t-test ({tail}{extra}) on {args.dataset} between {args.method1} and {args.method2}:")
