...
遍历候选 ``lambda`` 和 ``mu`` 组合，调用:func:`experiments.eval_example_3_7_shuffle.evaluate_accuracy` 评估准确率，输出最佳参数。提供 ``--debug`` 选项仅尝试前两组组合。
"""
...
from experiments.eval_example_3_7_shuffle import evaluate_accuracy
...
def search(
    ...
    for lam, mu in pbar:
        ...
        acc = evaluate_accuracy(shuffle_times)
...
if __name__ == "__main__":  # pragma: no cover
    ...
    shuffle_times = (
        args.times if args.times is not None else (5 if args.debug else 100)
    )  # 每组超参评估的重复次数
    ...
    results = search(pairs, shuffle_times=shuffle_times)
