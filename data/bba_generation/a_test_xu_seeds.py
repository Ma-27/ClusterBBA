...
验证 ``xu_seeds.py`` 与 ``kfold_xu_seeds.py`` 生成的 CSV 是否与原始 ``Seeds`` 数据集一致。
...
4. 给定特征组合 ``(15.26, 14.84, 0.871, 5.763, 3.312, 2.221, 5.22)`` 在两个 CSV 中对应的 ``sample_index``。
"""
...
# 依赖本项目内现成工具函数 / 模块
from data.bba_generation.kfold_xu_seeds import (
...
def _load_dataset() -> pd.DataFrame:
    """读取原始 Seeds 数据集并返回带标签的 ``DataFrame``。"""

    X_all, y_all, attr_names, _, full_class_names = load_seeds_data()
...
def _parse_bba(df: pd.DataFrame, attr_names: list[str]) -> dict[int, tuple[float, ...]]:
    ...
    for idx, grp in df.groupby("sample_index"):
        # ``grp`` 含 7 行属性，按 ``attr_names`` 排序还原为原始特征向量
        values = (
...
def _check_kfold_alignment(
    ...
    errors: list[str] = []
    # 将 ``sample_index`` 下的 7 行属性重组成特征向量，便于比对
...
def test_data_consistency() -> None:
    ...
    if not BASIC_CSV_PATH.exists():
        subprocess.run(["python", "-m", "data.bba_generation.xu_seeds"], check=True)
    if not KFOLD_CSV_PATH.exists():
        subprocess.run(["python", "-m", "data.bba_generation.kfold_xu_seeds"], check=True)
    ...
    target = (15.26, 14.84, 0.871, 5.763, 3.312, 2.221, 5.22)
