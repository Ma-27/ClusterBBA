...
基于 :mod:`xu_glass` 的实现，将 ``Glass`` 数据集划分为 ``K`` 折 (默认为 :data:`config.K_FOLD_SPLITS`=5)。在每个 ``test`` 折上顺时针取下一折作为``validation``，其余三折用于训练模型。随后分别对 ``validation`` 与 ``test`` 样本生成 BBA，保存至 ``kfold_xu_val_bba_glass.csv`` 与 ``kfold_xu_bba_glass.csv``。"""
...
CSV_PATH_TEST = Path(__file__).resolve().parents[1] / "kfold_xu_bba_glass.csv"
CSV_PATH_VAL = Path(__file__).resolve().parents[1] / "kfold_xu_val_bba_glass.csv"

if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="生成 Glass 数据集 K 折交叉验证版 BBA"
    )
    parser.add_argument(
        "--random_state",
        type=int,
    ...
    # xu_glass.fit_parameters 需要访问属性名
    ...
    for test_fold in range(K_FOLD_SPLITS):
        ...
        val_dataset = GlassDataset(X_val, y_val, attr_names, class_names)
