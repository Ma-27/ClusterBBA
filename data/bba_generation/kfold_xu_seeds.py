...
基于 :mod:`xu_seeds` 实现的生成流程，对 ``Seeds`` 数据集进行 ``K`` 折交叉验证(默认为 :data:`config.K_FOLD_SPLITS`=5)。在每个折上重建 ``Box-Cox`` 变换和正态分布模型，按 Xu 等人的方法为每个样本、每个属性生成 BBA。生成的所有折被合并保存至 ``kfold_xu_bba_seeds.csv``，其中额外记录 ``fold`` 序号，``dataset_split`` 字段统一填写 ``test``；此外，每个外层折还会生成验证集 CSV，保存在 ``bba_validation/kfold_xu_bbavalset_seeds`` 目录下，其 ``dataset_split`` 字段统一填写 ``validation``。可通过 ``--random_state`` 参数指定交叉验证的随机种子。
"""
...
VAL_DIR = (Path(__file__).resolve().parents[1] / "bba_validation" / "kfold_xu_bbavalset_seeds")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="生成 Seeds 数据集 K 折交叉验证版 BBA")
    # 指定种子
    ...
    for fold in range(K_FOLD_SPLITS):
        ...
        for inner_fold in range(K_FOLD_SPLITS - 1):
            ...
            # 包装为 SeedsDataset, 复用已有生成 BBA 的 API
            train_ds_in = SeedsDataset(X_in_tr, y_in_tr, attr_names, class_names)
            val_ds = SeedsDataset(X_in_val, y_in_val, attr_names, class_names)
        ...
        val_csv_path = VAL_DIR / f"bbavalset_seeds_fold{fold}.csv"
