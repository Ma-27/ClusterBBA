    ...
    for samp_idx in tqdm(range(total_samples), desc="Generating BBA", ncols=PROGRESS_NCOLS):
        ...
        for j in range(n_attr):
            ...
            row = {
                ...
                '{C1}': round(mass_dict['{C1}'], 4),
                '{C2}': round(mass_dict['{C2}'], 4),
                '{C3}': round(mass_dict['{C3}'], 4),
                '{C1 ∪ C2}': round(mass_dict['{C1 ∪ C2}'], 4),
                '{C1 ∪ C3}': round(mass_dict['{C1 ∪ C3}'], 4),
                '{C2 ∪ C3}': round(mass_dict['{C2 ∪ C3}'], 4),
                '{C1 ∪ C2 ∪ C3}': round(mass_dict['{C1 ∪ C2 ∪ C3}'], 4),
            }
            rows.append(row)
...
if __name__ == '__main__':
    ...
    y_all = raw.iloc[:, 0].values.astype(int) - 1  # 调整为 0,1,2
