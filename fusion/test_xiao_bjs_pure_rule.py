# -*- coding: utf-8 -*-
"""test_xiao_bjs_pure_rule.py

数值示例验证 — 纯 BJS 组合规则
================================

用法：
    python test_xiao_bjs_pure_rule.py [csv_filename]

若未指定 CSV, 默认读取的数据集可以手动更换。

CSV 约定：第一列为 ``BBA`` 标签, 其余列为焦元质量。
"""
...
if __name__ == "__main__":
    ...
    for k in range(2, len(raw_bbas) + 1):
        ...
        # _print_average(k, avg_bba)  # 如需查看平均 BBA, 取消注释

        combined_bba = xiao_bjs_pure_combine(cur_bbas)
        _print_step(k, combined_bba, crd)

    out_name = f"combined_xiao_bjs_pure_{os.path.splitext(csv_name)[0]}.csv"
